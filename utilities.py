...
def plot_first_id_fits_in_koi_data(koi_data_folder):
    """
    Counts ID folders under toi_data_folder, and only plots .fits files from the first ID folder found.
    """
    id_folders = [item for item in os.listdir(koi_data_folder)
                  if os.path.isdir(os.path.join(koi_data_folder, item))]
    print(f"Found {len(id_folders)} ID folders in {koi_data_folder}.")
    if not id_folders:
    ...
    id_path = os.path.join(koi_data_folder, id_folder)
...
# Example usage:
# my_dict = load_json_to_dict('fits_labels_dict.json')
def main():
    # mapping_hipparcos_catalog_nasaconfirmed()
    # combined_catalog = combine_toi_koi('TOIs.csv', 'KOIs.csv', 'combined_catalog.csv')
    ...
    with open('fits_labels_dict.json', 'w') as f:
        json.dump(merged_label_dict, f, indent=2)
    print("Merged label dict saved to fits_labels_dict.json")
    ...
    # plot_first_id_fits_in_koi_data('koi_data')
    # my_dict = load_json_to_dict('fits_labels_dict.json')
    # print('done')
